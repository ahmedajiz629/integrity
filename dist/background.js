...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (isVerifyMessage(message)) {
        ...
        handleVerification(message)
            .then((result) => {
            if (typeof tabId === "number") {
                const nextState = result.ok && result.matches ? "verified" : "rejected";
...
function isGenerateMessage(message) {
    if (!message || typeof message !== "object") {
        return false;
    }
    const candidate = message;
    return (candidate.type === "GENERATE_PAGE_DIGEST" &&
        typeof candidate.url === "string" &&
        typeof candidate.algorithm === "string" &&
        (candidate.encoding === "base64" || candidate.encoding === "base64url"));
}
function isReportStateMessage(message) {
    if (!message || typeof message !== "object") {
        return false;
    }
    const candidate = message;
    return (candidate.type === "REPORT_TAB_STATE" &&
        typeof candidate.url === "string" &&
...
async function setTabVisualState(tabId, state) {
...
function createIconAssets(color) {
    const sizes = [16, 32];
    const assets = {};
